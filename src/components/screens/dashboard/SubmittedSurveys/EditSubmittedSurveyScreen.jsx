 ...
 // Transform KoBoToolbox API data to our format
    ...
    filteredSurvey.forEach((question) => {
      ...
      if (question.type === "begin_group") {
        groupStack.push({
          ...
          questions: [],
          label: question.label, // Preserve the label
        });
        ...
        if (groupStack.length > 0) {
          ...
          if (groupStack.length === 0) {
            transformedQuestions.push({
              ...
              label: completedGroup.label, // Pass label to group object
            });
          } else {
            currentGroup = groupStack[groupStack.length - 1];
            currentGroup.questions.push({
              ...
              relevant: completedGroup.relevant,
              questions: completedGroup.questions,
              label: completedGroup.label, // Pass label to nested group object
            });
          }
        }
        return;
  ...
  const fetchSurveyData = async () => {
    try {
      ...
      console.log("Second one transform ", transformed);
      setSurveyData(transformed);
      setLoading(false);
    } catch (err) {
      console.error("Error fetching survey:", err);
  ...
  const getRelevantQuestions = (questions, answers) => {
    ...
    questions.forEach((question) => {
      ...
      if (question.type === "group") {
        ...
        if (
          question.name === "F_Composition" ||
          question.$kuid === "wy5Mh3hro" ||
          question.name === "access_civil_documentation_male" ||
          question.$kuid === "OCGg4P6IN" ||
          question.name === "access_civil_documentation_female" ||
          question.$kuid === "F8dZxdRwh"
        ) {
          if (isRelevant(question, answers)) {
            relevantQuestions.push(question); // Add the group as a single step
          }
        } else {
          if (isRelevant(question, answers)) {
  ...
  const renderQuestionComponent = () => {
    ...
    if (
      ...
      (currentQuestion.name === "F_Composition" ||
        currentQuestion.$kuid === "wy5Mh3hro" ||
        currentQuestion.name === "access_civil_documentation_male" ||
        currentQuestion.$kuid === "OCGg4P6IN" ||
        currentQuestion.name === "access_civil_documentation_female" ||
        currentQuestion.$kuid === "F8dZxdRwh")
    ) {
      return (
        <FCompositionGroup
